...
public class Copied_BOJ_01149_RGBDistance {
	...
	static int Paint_cost(int N, int color) {
		...
		/*
		 * in case of the 'Red' ¡æ Cost[N][0] = min( Cost[N-1][1], Cost[N-1][2] ) +
		 * Cost[N][0]
		 * 
		 * in case of the 'Green' ¡æ Cost[N][1] = min( Cost[N-1][0], Cost[N-1][2] ) +
		 * Cost[N][1]
		 * 
		 * in case of the 'Blue' ¡æ Cost[N][2] = min( Cost[N-1][0], Cost[N-1][1] ) +
		 * Cost[N][2]
		 */
		return DP[N][color];
	}
