...
public class Copied_Programmers_84021_FillingPuzzlePieces {
	...
	private static int findBlock(List<List<int[]>> board, List<List<int[]>> table) {
		int size = 0;
	...
	private static boolean isRotate(List<int[]> board, List<int[]> table) {
		...
		for (int i = 0; i < 4; i++) { // table퍼즐 0, 90, 180, 270도 회전
			...
			// (0, 0)을 기준으로 좌표를 맞추기 위해서 첫 값을 나머지 값에서 빼주는 작업
	...
	private static void bfs(int currentX, int currentY, boolean[][] visited, int[][] graph, int blockOrEmpty,
			List<List<int[]>> list) {
